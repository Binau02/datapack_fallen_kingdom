...
execute if score creeper spawner_red matches 1.. as @e[type=marker,tag=spawner_red,limit=1] at @s run setblock ~10 ~1 ~-2 minecraft:oak_wall_sign[facing=south]{Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function fk:spawner/red/sign_creeper_activate"}}',Text2:'{"text":"desactivé","color":"red"}',Text3:'{"text":""}'}
