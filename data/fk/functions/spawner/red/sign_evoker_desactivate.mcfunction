...
execute if score evoker spawner_red matches 1.. as @e[type=marker,tag=spawner_red,limit=1] at @s run setblock ~9 ~1 ~-2 minecraft:oak_wall_sign[facing=south]{GlowingText:1b,Text1:'{"text":"","clickEvent":{"action":"run_command","value":"function fk:spawner/red/sign_evoker_activate"}}',Text2:'{"text":"disabled","color":"red"}',Text3:'{"text":""}'}
